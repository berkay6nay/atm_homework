...
from transaction import BalanceInquiry,CashDeposit,Withdraw,Transfer, ChangePin , CheckDeposit
    ...
    while True:
        ...
        if customer_in_question: ##Sorgulanan customer db'de var ise
            ...
            if atm1.authenticate_user(pin= card_pin , customer= customer_in_question): ##  auth işlemi gerçekleşirse transaction kısmına geçilebilir
                ...
                if(selected_transaction == 3): # Çek Yatırma
                    screen1.show_message("Lütfen çekinizi yatirin  \n")
                    check_num = keypad1.get_input("(Çek numarasini girin)")
                    check_amount = keypad1.get_input("(Çek miktarini girin)")
                    screen1.show_message("Çek taranıyor... \n")
                    screen1.show_message("Çekinizin miktari " + str(check_amount) + " mi? \n")
                    screen1.show_message("(EVET) \n")
                    atm1.make_transaction(transaction= CheckDeposit(check_number = check_num , bank_code=bank.get_bank_code() , creation_date=datetime.today().date() , account_id=customer_in_question.get_account().get_account_number(),amount=check_amount),
                                        customer=customer_in_question)
